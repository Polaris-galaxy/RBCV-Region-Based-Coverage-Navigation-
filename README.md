...
本仓库用于：在**含障碍的大栅格地图**上，把自由空间按**长方形分区**拆分后，求一组**等半径观测点（圆盘）**，使所有自由像素在**视线遮挡（visibility clipped）**约束下被覆盖，并尽量**减少观测点数量**。
...
核心算法在 `src/coverage_planner/`，采用**分区 + 近似覆盖**的工程化流水线：
...
| 路径 | 说明 |
|------|------|
| `src/coverage_planner/` | **离线覆盖规划器（Python）**：地图 IO、分区规划、可见性、Set Cover、ILS 精修 |
...
- **使用说明**：`src/USAGE.md`
- **算法推导**：`src/coverage_planner/docs/ALGORITHM.md`
- **文件索引**：`src/FILES.md`
