...
| `rbcv_bringup/` | ROS1：**组合 launch**、**两阶段流水线**（`rbcv_stage1_*` → `survey_zones.json`；`rbcv_stage2_*` → 语义探索计划）；依赖 `map_tools`、`coverage_regions_ros`；详见 **`rbcv_bringup/README.md`** |
| `exploration_core_cpp/` | **可选**：`rbcv_plan_tour`（C++）加速 `exploration` 闭合回路；见 **`exploration_core_cpp/README.md`** |
